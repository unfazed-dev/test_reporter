...
""");
  ...
  Future<void> _cleanupOldReports() async {
    // Clean up reports in test_coverages directory
    ...
    // Clean test_coverages
    if (await coveragesDir.exists()) {
      // Delete all existing coverage reports for this path
      await for (final file in coveragesDir.list()) {
        if (file is File) {
          final fileName = file.path.split('/').last;
          // Check if file matches our coverage report pattern for this path
          if (fileName.startsWith('${pathName}_tc@') ||
              fileName.startsWith('coverage_${pathName}__')) {
            // Also clean old format
            try {
              await file.delete();
              print('  🗑️  Removed old coverage report: $fileName');
            } catch (e) {
