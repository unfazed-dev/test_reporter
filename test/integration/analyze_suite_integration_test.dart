/// Integration tests for analyzeSuite binary
///
/// Tests real execution of the analyzeSuite binary with actual
...
void main() {
  ...
  group('Process Execution Tests', () {
    test('should start and exit successfully', () async {
      final result = await executor.runBinary('analyze_suite', ['--help']);
    ...
    test('should handle invalid arguments gracefully', () async {
      final result =
          await executor.runBinary('analyze_suite', ['--invalid-flag']);
      expect(result, fails);
    ...
    test('should capture stdout and stderr separately', () async {
      final result =
          await executor.runBinary('analyze_suite', ['--verbose', '--help']);
  ...
  group('File I/O Tests', () {
    test('should create report file', () async {
      ...
      final result = await executor.runBinary('analyze_suite', ['test']);

      expect(result, succeeds);
      expect(
        Directory('tests_reports/suite'),
    ...
    test('should create subdirectory structure', () async {
      ...
      await executor.runBinary('analyze_suite', ['test']);

      expect(await Directory('tests_reports/suite').exists(), isTrue);
    });
    ...
    test('should embed JSON in report', () async {
      ...
      final reportFiles = Directory('tests_reports/suite')
          .listSync()
          .whereType<File>()
  ...
  group('CLI Arguments Tests', () {
    test('should accept --runs flag', () async {
      final result =
          await executor.runBinary('analyze_suite', ['test', '--runs']);
    ...
    test('should accept --verbose flag', () async {
      final result =
          await executor.runBinary('analyze_suite', ['test', '--verbose']);
      // Flag should be accepted (may succeed or fail based on other factors)
      expect(result.exitCode, anyOf(equals(0), equals(1)));
    });
    ...
    test('should accept --parallel flag', () async {
      final result =
          await executor.runBinary('analyze_suite', ['test', '--parallel']);
      // Flag should be accepted (may succeed or fail based on other factors)
      expect(result.exitCode, anyOf(equals(0), equals(1)));
    });
    ...
    test('should accept --max-workers flag', () async {
      final result =
          await executor.runBinary('analyze_suite', ['test', '--max-workers']);
      // Flag should be accepted (may succeed or fail based on other factors)
      expect(result.exitCode, anyOf(equals(0), equals(1)));
    });

    test('should accept --skip-coverage flag', () async {
      final result = await executor
          .runBinary('analyze_suite', ['test', '--skip-coverage']);
      // Flag should be accepted (may succeed or fail based on other factors)
      expect(result.exitCode, anyOf(equals(0), equals(1)));
    });

    test('should accept --skip-tests flag', () async {
      final result =
          await executor.runBinary('analyze_suite', ['test', '--skip-tests']);
      // Flag should be accepted (may succeed or fail based on other factors)
      expect(result.exitCode, anyOf(equals(0), equals(1)));
    });

    test('should accept --skip-failures flag', () async {
      final result = await executor
          .runBinary('analyze_suite', ['test', '--skip-failures']);
      // Flag should be accepted (may succeed or fail based on other factors)
      expect(result.exitCode, anyOf(equals(0), equals(1)));
    });

    test('should handle multiple flags together', () async {
      final result = await executor.runBinary('analyze_suite', [
        'test',
        '--runs',
        '--verbose',
      ]);
